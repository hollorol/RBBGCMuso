...
compose <- function(expr){
  ...
  # This tryCatch will give a much more specific error if a variable
  # (like 'projLAI') is not found in the simulation output data frame.
  lhsv <- tryCatch({
    eval(parse(text=lhs), envir=penv)
  }, error = function(e) {
    # Get variable names from the parent environment (the 'with' environment)
    var_names <- ls(envir = penv)
    
    # Create a more informative error message
    error_msg <- sprintf(
      "Error in compose(): Failed to evaluate variable '%s'.\nThis variable was not found in the simulation output data frame.\nAvailable variables are: %s\nOriginal error: %s",
      lhs,
      paste(var_names, collapse=", "),
      e$message
    )
    
    # Write to a log file for debugging (will be in the thread's root dir)
    write(paste(Sys.time(), error_msg, "\n---\n"), file = "compose_errors.log", append = TRUE)
    
    # Stop the execution with the new message
    stop(error_msg)
  })
  # --- END IMPROVEMENT ---
    ...
    future({
      tryCatch(
        ...
        , error = function(e){
          ...
          # --- IMPROVED ERROR HANDLING ---
          # This is the final catch-all. The error 'e' here is often
          # the *consequence* of an earlier failure (like the one you saw).
          # The new logging inside multiSiteThread and compose should catch the *root cause*.
          
          # We can add more context here too.
          error_msg <- sprintf(
            "FATAL ERROR in future block (thread %d):\nTime: %s\nError: %s\n\nThis is often a follow-on error.\nCheck for 'calibMuso_FAILURE.log' files in site directories (e.g., tmp/thread_%d/SITE_NAME/)\nCheck for 'compose_errors.log' or 'calcLikelihoods_errors.log' in thread directories (e.g., tmp/thread_%d/)\n",
            i, # 'i' is the thread number from the lapply
            Sys.time(),
            e$message,
            i,
            i
...
multiSiteThread <- function(measuredData, parameters = NULL, startDate = NULL,
  ...
  if(threadNumber == 1){
    ...
    origModOut <- lapply(resIterate, function(i){
      ...
      # --- IMPROVED ERROR HANDLING ---
      # This tryCatch now logs the specific error from calibMuso before returning NA
      res <- tryCatch(
        calibMuso(settings=settings, parameters=origEpc, silent = FALSE, skipSpinup = TRUE),
        error = function(e) {
          # Create a detailed error message
          log_message <- sprintf(
            "CRITICAL ERROR in 'calibMuso' (Original Run, threadNumber == 1)\nTime: %s\nSite: %s\nINI File: %s\nParameters (origEpc): %s\n\nError Message: %s\n---\n",
            Sys.time(),
            dirName,
            settings$iniInput[1],
            paste(origEpc, collapse=", "),
            e$message
          )
          ...
          # Write the error to a persistent log file *in the site directory*
          write(log_message, file = "calibMuso_FAILURE.log", append = TRUE)
          
          # Return NA so calibration can (try to) continue
          return(NA)
        }
      )
      # --- END IMPROVEMENT ---
  ...
  for(i in 2:(iterations+1)){
    ...
    # Get parameter names from the settings object for logging
    param_names <- settingsProto$calibrationPar 
    ...
    tmp <- lapply(resIterate, function(siteI){
      ...
      # This tryCatch now logs the specific error from calibMuso before returning NA
      res <- tryCatch(
        calibMuso(settings=settings, parameters=randValues[(i-1),], silent = TRUE, skipSpinup = TRUE),
        error = function(e) {
          # Get the current parameters for logging
          current_params <- randValues[(i-1),]
          
          # Create a detailed error message
          log_message <- sprintf(
            "CRITICAL ERROR in 'calibMuso' (Iteration %d)\nTime: %s\nSite: %s\nINI File: %s\n\nParameters (as values):\n%s\n\nParameters (as name = value):\n%s\n\nError Message: %s\n---\n",
            (i-1),
            Sys.time(),
            dirName,
            settings$iniInput[1],
            paste(current_params, collapse=", "),
            paste(sprintf("%s = %s", param_names, current_params), collapse="\n"),
            e$message
          )
          ...
          # Return NA so calibration can continue
          return(NA)
        }
      )
      # --- END IMPROVEMENT ---
...
calcLikelihoodsForGroups <- function(dataVar, mod, mes,
  ...
  if(!is.null(constraints)){
    # --- IMPROVED ERROR HANDLING ---
    # Check if 'm' (the simulation result) is a valid data.frame before
    # passing it to compoVect. 'mod' is a named list of simulation results.
    constRes<- sapply(names(mod), function(siteName) {
      m <- mod[[siteName]]
      
      # Check if the simulation result 'm' is a valid data frame
      if (!is.data.frame(m)) {
        # Log a warning that simulation likely failed for this site
        warning_msg <- sprintf(
          "Warning in calcLikelihoodsForGroups: Simulation data 'm' for site '%s' is not a data.frame (class: %s).\nThis almost certainly means the 'calibMuso' simulation for this site failed and returned NA.\nSkipping constraint check for this site.\n",
          siteName,
          class(m)[1]
        )
        warning(warning_msg)
        
        # Write to a log file (in the thread's root dir)
        write(paste(Sys.time(), warning_msg, "\n---\n"), file = "calcLikelihoods_errors.log", append = TRUE)
        
        # Return a vector of NAs matching the number of constraints
        # so sapply can combine the results.
        return(rep(NA, nrow(constraints))) 
      }
      
      # If 'm' is a data frame, proceed with constraint check
      compoVect(m, constraints)
    })
    # --- END IMPROVEMENT ---
    
    # Filter out NA columns that may have resulted from failed sites
    constRes_filtered <- constRes[, !apply(is.na(constRes), 2, all), drop = FALSE]
    
    if (ncol(constRes_filtered) > 0) {
      failType <- constMatToDec(constRes_filtered)
      constPass <- ifelse((100 * sum(apply(constRes_filtered, 2, prod), na.rm=TRUE) / ncol(constRes_filtered)) >= th, 1, 0)
    } else {
      # All sites failed the check or returned NA
      failType <- -1 # Or some other indicator of total failure
      constPass <- 0
    }
    ...
    failType <- 0
    constPass <- 1 # Pass constraints if there are none
  }
  ...
  likelihoodRMSE <- sapply(names(dataVar),function(key){
    ...
    # Add check for NA/Inf values before calculating likelihood/RMSE
    valid_indices <- !is.na(modelled) & !is.na(measured$mean) & is.finite(modelled) & is.finite(measured$mean)
    if (sum(valid_indices) == 0) {
      # No valid data to compare
      res <- c(NA, NA)
    } else {
      modelled_valid <- modelled[valid_indices]
      measured_valid <- measured$mean[valid_indices]
      res <- c(likelihoods[[key]](modelled_valid, measured_valid),
               sqrt(mean((modelled_valid - measured_valid)^2))
      )
    }
    
    
    # print(abs(mean(modelled, na.rm=T)-mean(measured$mean, na.rm=T))) # Use na.rm=T
    res
  })
...
spatialRun <- function(settingsProto,calibrationPar, parameters, calTable){
  ...
  modOut <- lapply(resIterate, function(i){
    ...
    # --- ADDING ERROR HANDLING HERE TOO ---
    # Good practice to have logging on this final run as well.
    res <- tryCatch(
      calibMuso(settings=settings, parameters=parameters, silent = TRUE, skipSpinup = TRUE),
      error = function(e) {
        log_message <- sprintf(
          "CRITICAL ERROR in 'calibMuso' (spatialRun - Final Best Parameter Run)\nTime: %s\nSite: %s\nINI File: %s\nParameters: %s\n\nError Message: %s\n---\n",
          Sys.time(),
          dirName,
          settings$iniInput[1],
          paste(sprintf("%s = %s", names(parameters), parameters), collapse="\n"),
          e$message
        )
        warning(log_message)
        write(log_message, file = "calibMuso_FINAL_RUN_FAILURE.log", append = TRUE)
        return(NA)
      }
    )
    # --- END IMPROVEMENT ---
