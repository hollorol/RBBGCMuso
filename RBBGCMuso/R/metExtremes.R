...
metReportMuso <- function(metFileName, skip = 4, numericReport = FALSE){
 ...
 if(!numericReport){
   cat("Daily and yearly extremes of meteorological data for the time period of", timeFrame[1], "-", timeFrame[2], ":\n
      Precipitation data:
      ...
      Temperature data:
      Lowest and highest daily temperatures (Tmin and Tmax): ", round(min(metTable$V4, na.rm=TRUE), digits=1), "deg C and", round(max(metTable$V3, na.rm=TRUE), digits=1), "deg C.
      Minimum and maximum of yearly averages (based on Tday):", round(min(yearlyTempAvg, na.rm=TRUE), digits=1), "deg C and", round(max(yearlyTempAvg, na.rm=TRUE), digits=1), "deg C.\n

      Solar radiation data:
      Minimum and maximum of daily values:      ", round(min(metTable$V8, na.rm=TRUE), digits=1), "W m-2 and", round(max(metTable$V8, na.rm=TRUE), digits=1), "W m-2.     
      Averages of the shortest and longest days:", round(mean(metTable$V8[na.omit(metTable$V9)==min(metTable$V9, na.rm=TRUE)]), digits=1), "W m-2 and", round(mean(metTable$V8[na.omit(metTable$V9)==max(metTable$V9, na.rm=TRUE)]), digits=1), "W m-2.\n
   ...
   report["temperature"] <- list(data.frame(minimum = c(daily = intMin(metTable$V4),
                                         ...
                                         maximum = c(daily = intMax(metTable$V4),
